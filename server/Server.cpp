...
{
    std::ofstream output(path, std::ios::binary); // файл куда читаем
    if (!output.is_open())
    {
        throw std::runtime_error("Не удается открыть файл для записи"); // проверяем, открылся ли файл
    }

    boost::system::error_code error; // ошибки
...
std::vector<int> Server::get_comands(boost::asio::ip::tcp::socket &socket)
{
    ...
    {
        int len = socket.read_some(boost::asio::buffer(comands), error);
...
int Server::run()
{
    ...
    try
    {
        ...
        boost::asio::ip::tcp::acceptor acceptor(io_context, boost::asio::ip::tcp::endpoint(boost::asio::ip::tcp::v4(), 6666));
...
// 2 если клиент не смог принять файл (например ввел какую-то хуйню вместо пути для сохранения)